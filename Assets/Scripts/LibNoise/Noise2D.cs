...
namespace LibNoise
{
    ...
    public class Noise2D : IDisposable
    {
        ...
        public void GetNormalizedData(out float[,] result, bool isCropped = true, int xCrop = 0, int yCrop = 0)
        {
            GetData(out result, isCropped, xCrop, yCrop, true);
        }
        ...
        public void GetData(out float[,] result, bool isCropped = true, int xCrop = 0, int yCrop = 0, bool isNormalized = false)
        {
            int width, height;
            float[,] data;
            if (isCropped)
